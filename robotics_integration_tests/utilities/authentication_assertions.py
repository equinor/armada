...
def assert_token_for_another_service_is_rejected(
    ...
    The token is valid in every respect except its audience -- it carries every
    role -- so this exercises audience validation rather than signature checking
    or authorisation. The audience follows from the requested scope: each API
    scope in the realm carries one audience mapper.
    """
...
def assert_missing_role_is_rejected(
    ...
    The token holds Role.User.HUA and nothing else. That matters: Flotilla
    answers 403 for a recognised principal with an insufficient role, but 401
    for a token carrying no roles at all, so a token with an empty role set
    would prove something weaker than intended.
    """
